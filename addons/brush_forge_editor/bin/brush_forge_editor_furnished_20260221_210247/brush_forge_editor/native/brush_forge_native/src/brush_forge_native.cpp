...
void BrushForgeNative::_bind_methods() {
	...
	ClassDB::bind_method(
		...
		DEFVAL(14.0f));
	ClassDB::bind_method(
		D_METHOD("fit_plane_from_points", "points", "reference_normal"),
		&BrushForgeNative::fit_plane_from_points);
	ClassDB::bind_method(
		D_METHOD("face_center", "plane_normals", "plane_distances", "vertices_world", "face_index", "epsilon"),
		&BrushForgeNative::face_center,
		DEFVAL(0.02f));
	ClassDB::bind_method(
		D_METHOD("align_copied_uv_transform", "src_basis", "dst_basis", "src_uv", "anchor_world"),
		&BrushForgeNative::align_copied_uv_transform);
	ClassDB::bind_method(
		D_METHOD("compute_bounds_from_vertices", "vertices_world", "min_extent"),
		&BrushForgeNative::compute_bounds_from_vertices,
		DEFVAL(0.001f));
	ClassDB::bind_method(
		D_METHOD("uv_basis_from_face_index", "face_index"),
		&BrushForgeNative::uv_basis_from_face_index);
	ClassDB::bind_method(
		D_METHOD("face_basis_from_index", "face_index"),
		&BrushForgeNative::face_basis_from_index);
	ClassDB::bind_method(
		D_METHOD("ray_plane_hit_normal", "ray_origin", "ray_dir", "plane_origin", "plane_normal"),
		&BrushForgeNative::ray_plane_hit_normal);
	ClassDB::bind_method(
		D_METHOD("ray_plane_hit_y", "ray_origin", "ray_dir", "plane_y"),
		&BrushForgeNative::ray_plane_hit_y);
	ClassDB::bind_method(
		D_METHOD("snap_float", "value", "step"),
		&BrushForgeNative::snap_float);
	ClassDB::bind_method(
		D_METHOD("snap_vector", "v", "step"),
		&BrushForgeNative::snap_vector);
	ClassDB::bind_method(
		D_METHOD("world_units_per_pixel_at", "distance_to_world_pos", "camera_fov_degrees", "viewport_height"),
		&BrushForgeNative::world_units_per_pixel_at);
	ClassDB::bind_method(
		D_METHOD("axis_delta_from_screen", "screen_origin", "screen_axis_tip", "mouse_pos", "drag_start_mouse"),
		&BrushForgeNative::axis_delta_from_screen);
	ClassDB::bind_method(
		D_METHOD(
			...
			"delta"),
		&BrushForgeNative::apply_vertex_drag_delta);
