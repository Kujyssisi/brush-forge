...
var vertex_drag_start_plane_normals := PackedVector3Array()
var vertex_drag_start_plane_distances := PackedFloat32Array()
...
func _to_packed_int32_array(values: Array) -> PackedInt32Array:
...
func _on_vertex_tool_toggled(enabled: bool) -> void:
	...
	if not enabled:
		...
		vertex_drag_start_plane_normals = PackedVector3Array()
		vertex_drag_start_plane_distances = PackedFloat32Array()
...
func _align_copied_uv_transform(src_brush: BrushForge, src_face_index: int, dst_brush: BrushForge, dst_face_index: int, copied_uv: Dictionary, anchor_world: Variant = null) -> Dictionary:
	...
	var native_uv = EDITOR_STATE_NATIVE_BRIDGE_SCRIPT.align_copied_uv_transform(src_basis, dst_basis, copied_uv, anchor)
	if native_uv is Dictionary:
		return native_uv
	_report_native_runtime_error_once("align_copied_uv_transform", "invalid native return type")
	return copied_uv
...
func _face_center_world(brush: BrushForge, face_index: int) -> Vector3:
	...
	if plane_data.is_empty():
		_report_native_runtime_error_once("face_center", "missing brush plane cache")
		return brush.position
	var normals: PackedVector3Array = plane_data["normals"]
	var distances: PackedFloat32Array = plane_data["distances"]
	var native_vertices := _to_packed_vector3_array(verts)
	var center = EDITOR_STATE_NATIVE_BRIDGE_SCRIPT.face_center(normals, distances, native_vertices, face_index, 0.02)
	if center is Vector3:
		return center
	_report_native_runtime_error_once("face_center", "invalid native return type")
	return brush.position
...
func _begin_vertex_drag(camera: Camera3D, mouse_pos: Vector2, alt_pressed: bool, start_anchor: Vector3) -> void:
	...
	var plane_data := _get_brush_plane_arrays(brush)
	vertex_drag_start_plane_normals = plane_data.get("normals", PackedVector3Array())
	vertex_drag_start_plane_distances = plane_data.get("distances", PackedFloat32Array())
...
func _update_vertex_drag(camera: Camera3D, mouse_pos: Vector2, alt_pressed: bool) -> void:
	...
	var native_vertices := _to_packed_vector3_array(vertex_drag_start_vertices)
	var selected_indices := _to_packed_int32_array(selected_vertex_indices)
	var plane_indices := _to_packed_int32_array(vertex_drag_plane_indices)
	var native_result = EDITOR_STATE_NATIVE_BRIDGE_SCRIPT.apply_vertex_drag_delta(
		native_vertices,
		selected_indices,
		plane_indices,
		vertex_drag_plane_vertex_indices,
		vertex_drag_start_plane_normals,
		vertex_drag_start_plane_distances,
		delta
	)
	if not (native_result is Dictionary):
		_report_native_runtime_error_once("apply_vertex_drag_delta", "invalid native return type")
		return
	var result_dict: Dictionary = native_result
	var out_normals: Variant = result_dict.get("plane_normals", null)
	var out_distances: Variant = result_dict.get("plane_distances", null)
	if not (out_normals is PackedVector3Array) or not (out_distances is PackedFloat32Array):
		_report_native_runtime_error_once("apply_vertex_drag_delta", "missing plane arrays")
		return
	var normals: PackedVector3Array = out_normals
	var distances: PackedFloat32Array = out_distances
	if normals.size() != distances.size():
		_report_native_runtime_error_once("apply_vertex_drag_delta", "plane array size mismatch")
		return
	brush.planes.clear()
	for i in range(normals.size()):
		brush.planes.append(NeoPlane.new(normals[i], distances[i]))
...
func _refresh_brush_bounds_from_planes(brush: BrushForge, mesh: MeshInstance3D) -> void:
	...
	if vertices.is_empty():
		...
		return
	var native_vertices := _to_packed_vector3_array(vertices)
	var bounds = EDITOR_STATE_NATIVE_BRIDGE_SCRIPT.compute_bounds_from_vertices(native_vertices, grid_size)
	if not (bounds is Dictionary):
		_report_native_runtime_error_once("compute_bounds_from_vertices", "invalid native return type")
		return
	var bounds_dict: Dictionary = bounds
	if not bool(bounds_dict.get("valid", false)):
		_report_native_runtime_error_once("compute_bounds_from_vertices", "native reported invalid bounds")
		return
	var center_raw: Variant = bounds_dict.get("center", null)
	var size_raw: Variant = bounds_dict.get("size", null)
	if not (center_raw is Vector3) or not (size_raw is Vector3):
		_report_native_runtime_error_once("compute_bounds_from_vertices", "missing center/size")
		return
	brush.position = center_raw
	brush.size = size_raw
	mesh.position = brush.position
	mesh.mesh = _build_brush_mesh(brush)
